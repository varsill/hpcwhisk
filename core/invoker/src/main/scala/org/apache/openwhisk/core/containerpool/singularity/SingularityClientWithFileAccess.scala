...
class SingularityClientWithFileAccess(singularityHost: Option[String] = None,
                                 containersDirectory: File = Paths.get("containers").toFile)(
  executionContext: ExecutionContext)(implicit log: Logging, as: ActorSystem)
    extends SingularityClient(singularityHost)(executionContext)
    with SingularityApiWithFileAccess {

  implicit private val ec = executionContext

  /**
   * Provides the home directory of the specified Singularity container.
   *
   * Assumes that property "containersDirectory" holds the location of the
   * home directory of all Singularity containers. Default: directory "containers"
   ...
   */
  // Singularity remake - unverified if it is used for anything meaningful now
  ...
  /**
   * Provides the configuration file of the specified Singularity container.
   *
  ...
  /**
   * Provides the log file of the specified Singularity container written by
   * Singularity's JSON log driver.
   *
   * Assumes that the file has the well-known location and name.
   *
   * Does not verify that the returned file actually exists.
  ...
  /**
   * Provides the contents of the specified Singularity container's configuration
   * file as JSON object.
   *
  ...
  /**
   * Extracts the IP of the container from the local config file of the singularity daemon.
   *
...
trait SingularityApiWithFileAccess extends SingularityApi {
